...
void MotorControl::SendTauCommand(uint32_t canID, double tau) {
    ...
    if (result == -1) {
        std::cerr << "Failed to send CAN frame. huihReturn value: " << result << std::endl;
    }
}
...
void MotorControl::SendHybridCommand(uint32_t canID, double pos, double vel, double kp, double kd, double tau) {
    ...
    int32_t result = sendUSBCAN(device, _channel, &sendInfo, data);
    if (result == -1) {
        std::cerr << "Failed to send CAN frame.uigi Return value: " << result << std::endl;
    }
}
...
void MotorControl::SendTorqueControlCommand(uint32_t canID, double target_pos , double current_pos, double velocity)
{
    double stiffness = 10;
    double damping = 1;
    ...
    //力控模型 力矩 = 刚度 * 位置误差 + 阻尼 * 速度误差
    double tau = stiffness * position_error - damping * velocity;
    if (tau > 2)
        tau = 2;
    if (tau < -2)
        tau = -2;
...
double MotorControl::compute_torque(double desired_positions,double current_positions,double current_velocities,double kp,double kd)
{
    double torque;
    torque = kp * (desired_positions - current_positions) - kd * current_velocities;
