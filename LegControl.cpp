...
void LegControl::SlowStandtoPosition(double time_step_, double standing_kp[12] , double standing_kd[12] ,double time ,bool init ){


    const double dt = time_step_ / 1000.0; // 转换为秒
    double init_joint_pos[12];
...
// 等待0.2秒稳定
    ...
    std::ofstream file("/home/bane/桌面/renxing/zhengxian_dandianji_time_kp10kd2.5.测试数据传输2.csv", std::ios::app); // 以追加模式打开文件
    file << std::fixed << time << ',' << init<< ',' <<target_joint_pos[4]<<','<<desired_pos[4] << ','<<tauff[4]<<','<< joint_positions_[4] << ',' << std::endl; // 写入数据
...
// 限制幅度
    if (joint_positions_[3]>1.15||joint_positions_[3]<-2.65){
    ...
    if (joint_positions_[4]<-2.1 || joint_positions_[4]>3.5){
        standing_kd[4] = 10;
        standing_kp[4] = 0.0;
